...
latchbrick_features = ['bat', 'latch', 'sleep']
latchbrick_versions = [['os', 1.0], ['all', 1.0], ['bat', 1.0], ['latch', 1.0], ['sleep', 1.0]]
...
class TestTempBrick(BaseCherryPyTestCase):
    def test_init(self):
        ...
        self.assertEqual(response.state['version']['os'], 0)
        self.assertIn('all', response.state['version'])
        self.assertEqual(response.state['version']['all'], 0)
        self.assertEqual(response.state['type'], None)

        # requested data is send over (without versions)
        response = self.webapp_request(f=tempbrick_features, x=1)
        self.assertIn('r', response.json)
        ...
        self.assertEqual(response.state['version']['temp'], 0)
        self.assertIn('bat', response.state['version'])
        self.assertEqual(response.state['version']['bat'], 0)
        self.assertIn('sleep', response.state['version'])
        self.assertEqual(response.state['version']['sleep'], 0)
