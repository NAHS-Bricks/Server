...
class TestTempBrick(BaseCherryPyTestCase):
    def test_init(self):
        ...
        self.assertEqual(response.state['last_bat_reading'], 3.7)
        self.assertNotEqual(response.state['last_bat_ts'], None)
    ...
    def test_delay_increase(self):
        ...
        self.assertEqual(response.state['delay_increase_wait'], 2)
        response = self.webapp_request(t=[['sensor1', 24]])
        self.assertEqual(response.state['delay_increase_wait'], 1)
        response = self.webapp_request(t=[['sensor1', 24]])
        ...
        self.assertEqual(response.state['delay'], 240)
        self.assertEqual(response.state['delay_increase_wait'], 3)
        response = self.webapp_request(t=[['sensor1', 24]])
        self.assertEqual(response.state['delay_increase_wait'], 2)
        response = self.webapp_request(t=[['sensor1', 24]])
        ...
        self.assertEqual(response.state['delay'], 300)
        self.assertEqual(response.state['delay_increase_wait'], 3)
        response = self.webapp_request(t=[['sensor1', 24]])
        self.assertEqual(response.state['delay_increase_wait'], 2)
        response = self.webapp_request(t=[['sensor1', 24]])
        self.assertEqual(response.state['delay_increase_wait'], 1)
        response = self.webapp_request(t=[['sensor1', 24]])
        self.assertEqual(response.state['delay_increase_wait'], 0)
        response = self.webapp_request(t=[['sensor1', 24]])
        self.assertEqual(response.state['delay_increase_wait'], 0)

    def test_delay_decrease(self):
        response = self.webapp_request(clear_state=True, v='1.0', f=tempbrick_features)
        response = self.webapp_request(b=3.7)
        response = self.webapp_request(t=[['sensor1', 24]])
        response = self.webapp_request(t=[['sensor1', 24]])
