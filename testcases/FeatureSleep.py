...
class TestFeatureSleepWithBat(BaseCherryPyTestCase):
    def test_charging(self):
        response = self.webapp_request(clear_state=True, v=self.v, d=60, b=4, p=11, y=['c'], s=1, m='1', a=850)  # a, m, p and s for getting rid of other features requests
        self.assertEqual(response.state['delay'], 60)
        self.assertIn('sleep_increase_wait', response.state)
        self.assertEqual(response.state['sleep_increase_wait'], 3)
    ...
    def test_charging_standby(self):
        response = self.webapp_request(clear_state=True, v=self.v, d=60, b=4.2, p=11, y=['s'], s=1, m='1', a=850)  # a, m, p and s for getting rid of other features requests
        self.assertEqual(response.state['delay'], 60)
        self.assertIn('sleep_increase_wait', response.state)
        self.assertEqual(response.state['sleep_increase_wait'], 3)
    ...
    def test_admin_override_delay_charging(self):
        response = self.webapp_request(clear_state=True, v=self.v, d=60, b=4.2, p=11, y=['c'], s=1, m='1', a=850)  # a, m, p and s for getting rid of other features requests
        self.assertEqual(response.state['delay'], 60)
        self.assertIn('sleep_increase_wait', response.state)
        self.assertEqual(response.state['sleep_increase_wait'], 3)
...
class TestFeatureSleepWithBatSolarCharging(BaseCherryPyTestCase):
    ...
    def test_solar_charging_standby(self):  # expected to not effect delay
        response = self.webapp_request(clear_state=True, v=self.v, d=60, b=4, y=['s'], m='1', a=850)  # m and a for getting rid of other features requests
        response = self.webapp_request(path='/admin', command='set', brick='localhost', key='bat_solar_charging', value=True)
...
class TestFeatureSleepWithLatch(BaseCherryPyTestCase):
    def test_static_delay_after_requests_done(self):
        ...
        response = self.webapp_request(x=2, m='1', a=850)  # m and a for getting rid of other features requests
        ...
        self.assertIn('d', response.json)
        self.assertEqual(response.json['d'], 10)
        response = self.webapp_request(m='1', a=850)  # m and a for getting rid of other features requests
    ...
    def test_short_delay_after_triggerstate_receivement(self):
        response = self.webapp_request(clear_state=True, v=self.v, l=[0, 1], d=60, b=4, s=1, m='1', a=850)  # a, m, b and s for getting rid of other features requests
        self.assertNotIn('r', response.json)
        self.assertIn('d', response.json)
        self.assertEqual(response.json['d'], 900)
