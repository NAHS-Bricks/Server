...
def __process_t(brick_new, brick_old):
    if 'temp' in brick_new['features'] and 'temp' in brick_old['features']:
        ...
        for sensor in [sensor for sensor in brick_new['last_temps'] if sensor in brick_old['last_temps']]:
            diff = abs(brick_old['last_temps'] - brick_new['last_temps'])
            max_diff = diff if diff > max_diff else max_diff
        if max_diff > 0.25 and new_brick['delay'] > 60:  # TODO: eventuell abhaengig von presicion machen
            new_brick['delay'] = 60
            new_brick['delay_increase_wait'] = 3
            return 'update_delay'
        elif max_diff > 0.25:  # Wenn delay schon auf 60 ist muss kein update gesendet werden, da die nur unnoetig rechenzeit braucht
            new_brick['delay_increase_wait'] = 3
            return None
        elif new_brick['delay_increase_wait'] <= 0 and new_brick['delay'] < 300:
            new_brick['delay'] += 60
            new_brick['delay_increase_wait'] = 3
            return 'update_delay'
...
def __process_b(brick_new, brick_old):
    if 'bat' in brick_new['features']:
        if brick_new['last_bat_reading'] < 3.5:
            print('Charge bat on ' + brick['id'] + ' (' + brick['desc'] + ')')
...
class Tempserver(object):
    ...
    def index(self):
        if 'json' in dir(cherrypy.request):
            ...
            brick = bricks[brick_id].copy()
            brick['last_ts'] = str(int(time.time()))
